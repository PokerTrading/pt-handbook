...
大多数策略教你看懂市场，但没人教你怎么下注、怎么管、什么时候走。Poker Trading 用拍卖理论读结构，用扑克规则管执行。像职业牌手一样，不预测，只打好手里的牌。
...
交易和德州扑克就是同一类游戏——都是在信息不完全的环境下做概率决策。
...
**极值完成（Excess）**——当拍卖到达极端价格时，如果那个区域的成交量极低，说明价格刚到那里就被快速拒绝了——拍卖干脆利落地结束了。VP的尾部呈现尖锐形态，这叫极值完成。Excess是VP尾部放量拒绝的结构证据，意味着那个边界非常牢固。上方的Excess叫**Excess High**（看空信号），下方的叫**Excess Low**（看多信号）。在实际操作中，Poker Table会自动识别和标记Excess——你不需要自己测量尾部形态。
...
除了日、周、月这些标准周期，Poker Table还支持按交易时段划分——比如东京时段、伦敦时段、纽约时段，每个时段各生成一张独立的VP。时段VP在日内交易中特别有用，就是把一天切成几张小桌，每张桌有自己的VA和关键位。读法和标准VP完全一样，只是时间窗口更短。
...
> 🏁 **你现在能做什么：** 你认识了读拍卖的三个工具——Table画结构、Flow看方向、Radar捕信号。下一章开始，你会学到怎么用这些信息做决策：坐哪张桌、往哪个方向看。
# 第三章 · 坐哪桌
...
偏见桌主要提供偏见方向，背景桌主要提供共振参考，两者可互补。共振价值按跨周期距离排序：偏见桌共振>背景桌共振（距离越大越有价值）。
...
**人话版本：** 信号出来了→(1)价格有没有往正确方向动？没动=扔掉 (2)动了，在新方向待够时间不回来（日内桌90分钟）？没有=扔掉 (3)还出了同方向的新推进且不被Radar否定？有=直接挂甜点位。没有=等价格回来测试不破再挂。
...
**快速通道：** 有时市场不给你慢慢等时钟门槛的机会——去程中直接出现同向AC且不被Radar否定（如dRE[PL]后立刻uAC[IBL]）。这个AC同时完成了三件事：AC本身=方向确认，AC=能量的定义，而Radar的确认机制（2-3根K线内价格没有反向突破信号K线极值）已经帮你做了时间检验。三关同时过=验牌完成→先手入场，直接找甜点位挂单。
...
**判断标准：价格持续停留在新方向≥下表时钟时间，不回到信号位。** 实盘中通过数K线来判断（更直觉），门槛以时钟时间为准——同一张桌不管用什么K线粒度，等待时间一样。
...
**例外：去程中出现同向AC且不被Radar否定 → 不用等满时钟时间。** Radar的否定机制本身就是时间检验——信号发生后2-3根K线内价格没有反向突破，就是不被否定，就是时间过了。直接跳到"验牌完成，先手入场"（见上方快速通道）。

### ③ 能量（先手 or 后手）

方向+时间都过了，信号大概率为真。方向+时间证明信号位有力量在防守——价格被挡住了，而且挡了一段时间。现在看有没有后续推进——能量证明这股力量还在进攻——挡住了，而且往前推了。**有没有能量，决定你怎么入场。**
...
5. 锚点被击穿 → 不做
6. 价格不回踩直接走 → 甜点位够得到？挂单等（见Ch6 §6.1挂单与有效期）。够不到？评估延续牌（Ch5）
...
| **② 时间** | ≥时钟门槛不回到信号位 | ≥时钟门槛不回到突破位 | ≥时钟门槛不回到Excess区域 |
...
> 验牌在你所在牌桌的K线上进行，时钟时间门槛见§4.2②时间表。不切到更小K线上"验得更快"——K线粒度不同但同桌时钟门槛一样。
>
> 验牌没有时间限制——唯一的过期条件是VP更新后边界位移。如果VP没更新、边界没动，即使信号过了6小时你才打开图表，仍然可以回溯后续K线表现来补验。过期≠失败（不影响未来判断，可能开延续牌新局）。

### 场景

**RE · 后手入场（方向+时间过了，没能量）：** BTC 15min，多头桌风，dRE 5.3出现在PL附近。①方向：价格反弹了。②时间：价格持续待在PL上方≥6根K线（90分钟）不回去。③能量：没有同向uAC出现。→后手入场，等价格回踩PL附近、关键锚点不被击穿后再挂甜点位。
...
**Excess · 后手入场：** BTC 15min，多头桌风，没有uRE信号但Excess High成型=边界发牌。①方向：价格远离Excess区域向下走。②时间：价格持续远离≥6根K线（90分钟）不回去。③能量：没有同向dAC出现。→后手入场，等回踩Excess区域不破后再挂甜点位。
...
> **本章要点速记**
>
> 发牌 = 8个一级边界：Radar捕捉 PH/PL/VAH/VAL/IBH/IBL 的AC和RE（其中PH/PL/IBH/IBL事件触发才显示）+ Table识别 Excess High/Low（VP结构演化，始终可见）· VA内=噪音 · POC=辅助 · Radar和Excess共同存在以Excess为准
>
> 验牌三步（统一链路）：①方向（必须）→ ②时间≥时钟门槛不回去（分时段15min/日内90min/短线12h/波段3天）→ ③能量=去程中同向AC
>
...
牌型由**交易方向 vs 桌风方向**决定。两者一致=顺风，两者相反=逆风。

**顺风牌（主食）。** 交易方向和桌风方向一致。多头桌风下的做多、空头桌风下的做空，不管来自哪种入场逻辑（买便宜、追突破、假跌破反做多），都是顺风牌。新手应该集中精力打这个牌型。

**逆风牌。** 交易方向和桌风方向相反。天然需要更高门槛——更强信号、更谨慎仓位。
...
**延续牌。** 原始甜点位够不到（Ch6够不到规则）后的补救。不是独立方向判断，而是入场时机的处理方式。甜点位够得到时继续挂单等（见Ch6挂单与有效期），够不到后原始挂单作废——如果价格远离后在途中形成新小平衡区，在新平衡区边缘找甜点位入场。

小平衡区三个条件：3-5根K线横向震荡（非单边推进），幅度不超甜点位到TP1距离的30%，K线实体逐渐缩短、影线互相覆盖。

硬性前提：桌风没有反向 + 有清晰的新小平衡 + 小平衡内出现过一次顺风方向的Radar信号。三缺一就放弃。延续牌天然用更轻仓位。甜点位、SL和TP定位见Ch6（算笔账）。
...
**逆VA迁移降级：** VA迁移=当前VP的VA位置相对上一VP的VA位置变化，反映市场**此刻正在做的事**。桌风是回溯统计（过去N个周期），VA迁移是最新一期的原始数据。

规则：**交易方向逆VA迁移→牌力降一级。不逆→不降。VA没方向→不管。** 降级机制天然产生优先级——顺VA的牌不被降，逆VA的牌被降，你自然会优先选择顺VA方向的机会。

例：多头桌风，但VA向下迁移。做多信号出来→顺桌风但逆VA→顺风牌降一级（AA按KK打，KK按AK打，以此类推）。做空信号出来→逆桌风但顺VA→逆风牌不额外降（按逆风门槛正常走）。两相比较，做空那手牌更有吸引力——这就是VA迁移的调节作用。

"VA迁移有没有方向"不量化，一眼判断：VA位置明显变了=有方向，没怎么动=没方向。

牌型仍按桌风归类——VA迁移管降级，桌风管牌型。逆VA降级和逆风降级不叠加，取最严格的那个处理（都是"方向不利"的体现，不惩罚两次）。双Excess免VA迁移降级（结构穷尽信号优先于VA迁移统计）。详见速查卡10。
...
**双Excess特权：** 双Excess是方向穷尽的结构证据——同一区域两次探索且第二次更弱=拍卖在那个方向彻底结束了。两项豁免：①逆风牌免Radar 7+条件直接入池，②免逆VA迁移降级。纹理和R:R照常走。逆风降级照常——如果交易方向逆桌风，双Excess仍按逆风牌门槛处理（AA降9手，KK需三条件入池6手，以此类推），只是免掉了VA迁移那一层降级和7+评分门槛。
...
| **平衡桌风** | 🟢 标准入池 · 12手 | 🟡 轻仓入池 · 6手 | 🔴 多数弃牌（仅7+时3手） | 🔴 弃牌 |
...
- 延续牌天然轻仓
- 逆VA迁移→降一级（双Excess豁免）。逆VA降级和逆风降级不叠加，取最严格（详见速查卡10）
...
**降级来源汇总：**
- 逆VA迁移 → 降一级（双Excess豁免。逆VA降级和逆风降级不叠加，取最严格）
- 判断不确定 → 降一级
- 纹理湿润 → 降一级
- 纹理极端湿润 → 直接弃牌
...
**多重降级场景示例：** 多头桌风+VA下迁，做多信号AA，纹理湿润。逆VA迁移→降一级=KK，湿润纹理→降一级=KK。两个降级来源都指向KK，取最严格=KK（9手）。但因为有两个降级来源同时存在，仓位减半=5手（向下取整到4手也可）。如果只有一个来源（比如只有湿润纹理），就是KK 9手不减半。
...
>
> 牌型四种：顺风牌（顺桌风）· 逆风牌（逆桌风）· 诈唬牌（假突破反做，可顺可逆）· 延续牌（够不到后补救）
>
...
🥉 **斐波那契回撤。** 价格弹得太快、太远，验牌已经通过——回踩时既没有合适的结构锚点，信号K线开盘价也离得太远。这时候用斐波那契回撤作为参考：从信号起点到弹回最远点画一段距离，取0.5、0.618或0.786的回撤位。具体选哪个比例，见下方"斐波那契比例怎么选"。

> 斐波那契不需要深入理解原理——把它当成一种度量工具：信号运动了一段距离，价格回撤到这段距离的五成到八成位置时，往往是不错的入场点。
...
🥉 **斐波那契回撤。** 突破后价格强势推进、不回踩到突破位。用从突破点到推进最远点的斐波那契0.5/0.618/0.786作为备用甜点位。比例选择同RE，见下方。

### 斐波那契比例怎么选

RE和AC的🥉都用斐波那契，但0.5、0.618、0.786选哪个？**默认0.618。** 根据两个因素调整：

**看信号力度+运动距离。** 信号强（Radar 7+）且价格运动距离远→回撤浅→0.5。信号普通（5-7）→0.618。信号偏弱或距离近→回撤深→0.786。

**看运动路上的VP形态。** 价格穿过HVN（红色密集区）还能跑远=硬吃掉对手盘，真强势→0.5。价格穿过的是LVN（绿色/白色通道）=真空推进，对面没人→回撤可能更深→0.618或0.786。

| 条件 | 选0.5 | 选0.618 | 选0.786 |
|------|-------|---------|---------|
| 信号力度 | Radar 7+ | 普通（5-7） | 偏弱 |
| 运动距离+路况 | 远+穿过HVN | 中等距离 | 近，或远但穿的是LVN |
| R:R空间 | TP远，R:R宽裕 | 标准 | R:R紧张，需要更好入场价 |

0.5附近恰好有结构位（HVN边缘、VAH/VAL等）→优先用🥇结构锚点，Fib只是确认。三个比例都算不出达标R:R→弃牌。
...
甜点位候选可能不止一个——🥇🥈🥉各层候选，或同层多个结构锚点。你不一定要选一个候选全仓挂，**可以把计划手数分散到多个候选上挂单。** 分批挂单配合前置下注使用。

比如AA 12手，Fib 0.5挂6手 + Fib 0.618挂6手。或者结构锚点挂8手 + 信号K线开盘价挂4手。怎么分、分几档=牌手根据行情和经验自行判断。快速行情选近的候选集中挂，普通行情可以分散到多个候选。也可以完全不分，选一个候选全挂——和之前一样。

**三条硬规则：**

1. **每个挂单价独立过R:R门槛。** 不能因为"远的那档拉低了均价"就让近的那档蒙混过关。R:R不达标的候选不挂。
2. **SL统一。** 同一个信号，同一个失效点，SL只有一个。不同挂单价只影响各自的R:R，不影响SL位置。
3. **成交多少手就按多少手管理。** 12手计划只接到6手→按6手打，三组等比缩放（2/2/2）。没接到的手数不追加、不补挂。这不是踏空——这是风险没有被使用，下一手牌来了再说。

> 分批挂单不是追价——每一档都是提前挂好的限价单，都在甜点位候选上，不是市价追入。
...
> **本章要点速记**
>
> 甜点位 = RE三层（结构锚点>K线开盘价>Fib） · AC三层（突破位回踩>POC翻转>Fib） · Fib比例：默认0.618，信号强+距离远+穿HVN→0.5，信号弱或穿LVN→0.786，附近有结构位优先用结构 · 多个候选：信号强选近的，路上HVN选前方，LVN别选中间 · 分批挂单：计划手数可分散到多个候选上，每个独立过R:R，SL统一，接到多少打多少，没接到的不追加 · 够不到（VA外）或未成交→弃牌不追价 · 挂限价单 · 有效期=验牌通过起§6.1表格 · 提前失效：否定/反向AC通过验牌/桌风对立
>
...
**多重降级：** Ch5提到降级不叠加、取最严格的那个。但如果你有多个降级来源同时存在（比如逆VA迁移+纹理湿润），虽然牌力只降一级，仓位要额外减半。牌力决定"打不打"，仓位反映"多重不利因素叠加的风险"。例：原始牌力AA，逆VA迁移降到KK，同时纹理湿润——牌力按KK打，手数=9×50%=4.5→取整4手。
...
**分批挂单的成交管理：** 如果你在多个甜点位候选上分批挂单（Ch6），实际成交多少手就按多少手管理。AA 12手计划，只接到6手→按6手打，三组等比缩放（2/2/2）。没接到的手数不追加、不补挂——下一手牌来了再说。
...
>
> 仓位与下注：前置=一次入场（AA 4/4/4 · KK 3/3/3 · AK 2/2/2 · AQ 1/1/1）· 分街=轻仓+两次加注（AA 5/4/3 · KK 4/3/2）· 逆风/延续/平衡/新手→前置 · AA/KK顺风干燥→分街 · 分批挂单：接到多少手按多少手管理，三组等比缩放，没接到的不追加
>
...
| SL | $4,870 | 失效点=双Excess Low（穿过=信号失效）外侧。距离=$60。≥分时段下限$3 ✅ |
...
**时间：持续了吗？** 持续停留在新方向≥时钟门槛（日内桌90分钟），不回到信号位。没持续→弃牌。
...
**SL定位。** 甜点位附近有没有明确的结构支撑/阻力可以放SL？**没有结构就弃牌。** 极简版不用数学SL——没有结构说明入场位置不够好，别勉强。
...
**❌ SL没有结构保护。** 甜点位附近找不到明确的结构放SL = 入场位置不够好。别勉强。
...
| **② 时间** | 持续停留在新方向≥时钟门槛（分时段15min/日内90min/短线12h/波段3天），不回到信号位 | 验牌通过 | 弃牌 |
...
|------|------|------|
| 顺风牌 | 交易方向=桌风方向 | 标准（主食） |
| 逆风牌 | 交易方向逆桌风 | AA降级入池，KK条件入池（6手，7+ + 纹理不湿润 + R:R≥1.5:1，三条件缺一弃牌），AK降级3手 |
| 诈唬牌 | 假突破后反做（顺风诈唬/逆风诈唬） | 按方向归类：顺风=顺风门槛，逆风=逆风门槛 |
| 延续牌 | 错过原始入场后的小平衡补救 | 按初始牌力降一级，三前提缺一不做 |

**平衡桌风门槛（非独立牌型）：** 桌风平衡时无顺风/逆风之分，信号仍按四种牌型归类，但门槛介于顺风和逆风之间：AA标准入池，KK轻仓（6手），AK多数弃牌（仅7+时3手），AQ弃牌，R:R≥1.2:1。
...
逆VA迁移降级：交易方向逆VA迁移→降一级。不逆→不降。VA没方向→不管。逆VA降级和逆风降级不叠加，取最严格。双Excess免VA迁移降级。牌型仍按桌风归类。详见速查卡10。
降级来源：逆VA迁移 → 降一级（双Excess豁免），判断不确定 → 降一级，湿润纹理 → 降一级，极端湿润 → 直接弃牌。
...
## 甜点位（RE和AC分开）
RE甜点位：🥇结构锚点（IBH/IBL、VAH/VAL、POC）🥈信号K线开盘价 🥉斐波那契0.5/0.618/0.786
AC甜点位：🥇突破位本身（回踩入场）🥈POC翻转位 🥉斐波那契0.5/0.618/0.786
Fib比例选择：默认0.618。信号强（7+）+距离远+穿过HVN→0.5。信号弱或距离近或穿的是LVN→0.786。附近有结构位→优先用🥇。
分批挂单：计划手数可以分散到多个候选上挂限价单（比如Fib 0.5挂一半+Fib 0.618挂一半）。每个挂单价独立过R:R，SL统一，成交多少手按多少手管理，没接到的不追加。
"够不到"=甜点位在当前VA外（VA内含边界=够得到）。三层都够不到=不入场。
...
数学SL（降级方案）：仅限最终牌力AA/KK+Radar 7+（降级后的牌力，非原始牌力）。仓位降一级+R:R硬要求+0.5。AK/AQ不允许数学SL。Excess发牌无Radar评分，不适用数学SL。
...
| 验牌 (Check Hand) | 信号/事件验证 | 三步链路：①方向→②时间（≥时钟门槛：分时段15min/日内90min/短线12h/波段3天）→③能量（同向AC）。入场和加注通用 |
...
| 最终牌力 (Final Hand Strength) | 降级后的实际牌力 | 原始牌力 - 降级（逆VA迁移/不确定/纹理）= 实际执行等级 |
| 数学SL (Math Stop Loss) | 盈亏比反推的止损 | 无结构保护，纯数学位置。仅限AA/KK+Radar 7+，仓位降一级，R:R+0.5。AK/AQ不允许 |
| SL定位 | 唯一原则 | SL=最后确认失效点外侧+buffer。先手→能量K线极值，后手→回踩极值，延续牌→小平衡区反向极端位。有结构保护=高质量，无结构=有效但较弱 |
| 发牌有效期 | 牌的最大存活时间 | 验牌通过起：分时段=当前+下一时段，日内=今天+明天，短线=当前周+下周三收盘，波段=当月+下月D10。提前失效：Radar否定、反向AC且通过验牌、桌风对立。新VP同位置出新信号=新牌不是续命 |
| 降级 (Downgrade) | 牌力下调 | 来源：逆VA迁移→降一级（双Excess豁免，逆VA与逆风不叠加取最严），不确定→降一级，湿润纹理→降一级，极端湿润→弃牌。不叠加取最严，多来源→仓位减半 |
...
| 甜点位 (Sweet Spot) | 最佳入场价位 | RE=①结构锚点>②信号K线开盘价>③斐波那契。AC=①突破位回踩>②POC翻转位>③斐波那契。多个候选：信号强选近的，路上HVN选前方。三层都够不到或未成交=不入场不追价 |
...
| 平衡桌风 | 桌风状态（非独立牌型） | 回溯窗口内VA同向移动<60%。AA标准入池（12手），KK轻仓（6手），AK多数弃牌（仅7+时3手），AQ弃牌。R:R≥1.2:1 |
| 共振 (Confluence) | 跨周期关键位重合 | 执行桌和偏见桌/背景桌关键位价差≤VA宽度10%。偏见桌共振>背景桌共振 |
| 熔断 (Circuit Breaker) | 强制停止交易 | 笔数熔断：日止损≥2笔停/周止损≥4笔停+复盘。子弹余量<20%→停止当颗 |
| 洗牌期 | VP结构混乱期 | 当前VP和上一桌VP都看不清结构，方向不明，关键位互相矛盾。处理：不坐，等结构清晰再说（Ch3 §3.2） |
...
```
☐ 甜点位够得到
    甜点位在VA内(含边界)?    甜点=____  VAH=____  VAL=____
    不够 → 弃牌

☐ SL在失效点
    失效点=____  SL=____  距离=____ ≥下限?
    有结构保护? Y/N（质量标注，N=清醒入场）

☐ R:R ≥ 门槛
    顺风≥1.2 / 平衡≥1.2 / 逆风≥1.5
    TP1=____  R:R=____:1
    不达标 → 数学SL? (需AA/KK + 7+ · Excess不适用)
                不可用 → 弃牌
                可用 → 仓位降级 + R:R+0.5

→ 三个都 ☑ = Go · 进Ch7

挂单有效期见§6.1表格 · 否定/反向AC验牌通过/桌风对立→撤单
```
...
信号出现 → ① 方向
             ...
             No → 弃牌
          → ② 时间
             ≥时钟门槛待在新方向不回去（日内90min/短线12h/波段3天）
             回去了 → 弃牌
          → ③ 能量
             同向AC且不被Radar否定?
             有 → 先手入场（直接挂甜点位）
...
□ Ch5 牌型：顺风/逆风/诈唬/延续？
  ...
  □ 纹理：干燥/中性/湿润？
  □ 降级：逆VA迁移?/不确定?/纹理? → 最终牌力（卡10）
  □ 决策总表查表 → 🟢Go / 🔴弃牌
    ↓ Go
□ Ch6 甜点位：结构锚点>K线开盘价>Fib回撤 · 够不到→弃牌
  □ SL：失效点外侧+buffer（标准）或数学SL（降级）· ≥最小下限
  □ TP1：第一个结构阻挡
  □ R:R：≥门槛？ · 不达标→弃牌
    ↓ 达标
...
**桌风定门槛，逆VA降一级，双Excess最硬。**

### 三步决策（信号出来后）

```
❶ 桌风定门槛
   交易方向 vs 桌风 → 顺风牌（标准）/ 逆风牌（高门槛）→ 查决策总表（卡2）
...
❸ 双Excess特权
   双Excess → 免Radar 7+条件 + 免VA迁移降级。逆风降级照常
```

### 等牌阶段（信号出来前）

没发牌时盯桌风方向的边界等机会。进阶：盯偏见方向的边界（Ch3 §3.4）。

### 加分项

偏见=桌风=VA迁移=发牌方向 → 最高确信，标准仓位

偏见≠桌风 → 偏见管盯哪，桌风管怎么打（各管各）

偏见通过共振间接影响牌力（偏见桌关键位参与共振判定）
