...
【截图：第一组——VP图标注VA区间（蓝色阴影）、VAH/VAL（蓝色虚线）、POC（红色实线）】

> ✅ **检查点：** 如果你理解了"VA=大多数人觉得合理的价格区间"和"POC=成交最密集的那个价格"，就可以继续了。不确定的话回到上面再看一遍。

### 第二组：拍卖边界——Radar识别（PH/PL + IB）

**周期最高价与最低价（PH / PL）**——当前计算周期内价格触及的天花板和地板。它们是拍卖的极端边界——价格到了这里，要么被拒绝弹回，要么突破进入新领地。Poker Radar（第二章会详细讲）在K线级别实时捕捉PH/PL处的拒绝事件。

**初始平衡区（IB）**——开盘后初始阶段形成的价格范围，代表了市场的"开局博弈区"。具体的计算窗口因品种和周期而异，第二章会详细说明。IB的上界（**IBH**）和下界（**IBL**）在日内交易中是重要的参考关键位。

**价值区边界（VAH / VAL）**——VA的天花板和地板。Radar在这里捕捉接受和拒绝两种事件。

以上6个位置（PH、PL、VAH、VAL、IBH、IBL）全部由Poker Radar在K线级别实时识别。

【截图：第二组——VP图标注PH/PL（最顶/最底虚线）、VAH/VAL（蓝色虚线）和IBH/IBL（橙色虚线）】

> ✅ **检查点：** PH/PL=拍卖极端边界，VAH/VAL=价值区边界，IBH/IBL=开局博弈区边界。这6个位置由Radar在K线级别实时识别。
...
Excess和PH/PL都出现在周期的高低点区域，但识别方式完全不同——PH/PL由Radar在K线级别实时捕捉拒绝事件，Excess由Poker Table在VP级别识别成交量分布结构。PH是一条精确的价格线（周期最高价），Excess是一段尖锐的尾部区域（VP长出来的结构）。**两者可以单独出现，也可以共同存在，都是一级边界。**
...
【截图：第三组——VP图标注Excess（尖锐尾部）和Poor（肥厚尾部），对比PH/PL线的位置】

> ✅ **检查点：** Excess High/Low由Table在VP级别识别，和Radar识别的PH/PL是不同指标、不同机制观察同一片战场。Excess=门关了（一级边界，发牌），Poor=门没关（不发牌）。
...
```
        ┌─────────┐
        │ PH / PL │  ← Radar识别：拍卖极端边界
        │ Excess  │  ← Table识别：VP结构极端边界
        ├─────────┤
      ┌─┤ VAH/VAL ├─┐  ← Radar识别：价值区边界
      │ ├─────────┤ │
    ┌─┤ │ IBH/IBL │ ├─┐  ← Radar识别：开局博弈区
    │ │ └─────────┘ │ │
    └─┘             └─┘
```
...
>
> 一级边界（发牌位）：Radar识别 PH/PL/VAH/VAL/IBH/IBL + Table识别 Excess High/Low · POC = 参考位（不发牌）
>
...
三个工具之间有明确的分工边界。Poker Table画出VP结构并识别Excess边界和Poor标记，Poker Flow判断方向不捕捉信号，Radar在K线级别捕捉AC/RE事件并评分。Table和Radar各自负责一部分一级边界的识别——Table识别Excess High/Low（2个），Radar识别PH/PL/VAH/VAL/IBH/IBL（6个）。决策权留给你——后面的章节会教你怎么用这些信息做判断。
...
这就是Poker Radar的工作。它实时监控所有关键位，当价格触及某个关键位并形成接受或拒绝时，Radar会自动识别这个信号，给它评分，然后推送给你。你不需要盯着每一根K线去判断"这算不算一个信号"——Radar替你做了这件事。8个一级边界中的6个（PH/PL/VAH/VAL/IBH/IBL）由Radar在K线级别实时识别，另外2个（Excess High/Low）由Table在VP级别识别。
...
你刚走完的九章，每一步都在替换这套东西：用结构替换叙事，用规则替换直觉，用概率替换预测。同时接入其他指标或策略也不建议——不是它们不好，是信号源多了会打架，打架你就会犹豫，犹豫就会凭感觉选一个。一条决策链，从头走到尾，中间不掺别的。
...
| 验牌 (Check Hand) | 信号/事件验证 | 判断市场事件是不是真的，入场和加注通用 |
| 牌型 (Hand Type) | 交易类型 | 顺势牌/反转牌/诈唬牌/延续牌/中性牌 |
...
| 手数 (Hands) | 仓位单位 | 1手=1个风险单位（X ÷ SL距离 × 每点价值）。10手=满仓 |
...
| 边缘交易 (Edge Trading) | 只在一级边界发牌 | VA内部信号=噪音；一级边界=PH/PL/VAH/VAL/IBH/IBL |
| 一级边界 (Primary Edge) | VP结构的发牌位 | PH/PL（拍卖极值）+ VAH/VAL（价值区边界）+ IBH/IBL（开局博弈区边界）|
...
| 贪婪组 (Greed Group) | 博大行情部分 | 默认20%仓位。TP2后SL移TP1，等TP3/结构反转/对侧Excess下桌 |
| 结构自证错误 | 入场理由被推翻 | AC/RE事件被反向事件否定，SL尚未触发但方向已错→主动平仓 |
| 周期结束平仓 | VP周期即将结束时平仓 | 第四种合规平仓。日内桌=收盘前1h，短线桌=周五收盘前。非强制 |
| 中性牌 | 平衡桌风下的信号 | 无顺势/逆风之分。AA标准入池，KK轻仓，AK多数弃牌，AQ弃牌。R:R≥1.8:1 |
| 共振 (Confluence) | 跨周期关键位重合 | 执行桌和偏见桌/背景桌关键位价差≤VA宽度10%。偏见桌共振>背景桌共振 |
| 熔断 (Circuit Breaker) | 强制停止交易 | 日亏≥2%/周亏≥5%/月亏≥10%→对应级别停止 |
...
| Poor High | 上方弱极值 | 顶部拍卖不充分 |
| Poor Low | 下方弱极值 | 底部拍卖不充分 |
...
Radar的VP周期和IB时长与同一张图的Table保持一致，确保两个指标看的是同一组VP边界。
...
| 边界 | 结果 | 动作 | 牌型 | ⭐ |
|------|------|------|------|----|
| VAL/IBL/PL/ExL | RE弹回/Excess | 买便宜做多 | 顺势 | ⭐⭐⭐ |
| VAL/IBL | AC失败反弹 | 假跌破做多 | 顺势诈唬 | ⭐⭐⭐ |
| VAH/IBH | AC成功站稳 | 回踩追突破做多 | 顺势 | ⭐⭐ |
| VAH/IBH/PH/ExH | RE打回/Excess | 卖贵做空 | 反转 | ⭐ |
| VAH/IBH | AC失败回落 | 假突破做空 | 反转诈唬 | ⭐ |
| VAL/IBL | AC成功站稳 | 反弹追跌破做空 | 反转 | ⭐ |
...
| **顺势** | 🟢 10手 | 🟢 8手 | 🟡 6手 | 🟡 4手 |
| **顺势诈唬** | 🟢 10手 | 🟢 8手 | 🟡 6手 | 🟡 4手 |
| **反转** | 🟡 8手(降KK) | 🟡 6手 *三条件 | 🔴 弃 | 🔴 弃 |
| **反转诈唬** | 🟡 8手(降KK) | 🟡 6手 *三条件 | 🔴 弃 | 🔴 弃 |
| **中性（平衡）** | 🟢 10手 | 🟡 6手 | 🔴 (仅7+) | 🔴 弃 |
| **延续** | 🟡 4手 | 🟡 4手 | 🔴 弃 | 🔴 弃 |

**反转KK三条件：** 7+ ＋ 纹理不湿润 ＋ R:R≥2:1（缺一弃牌）
**双Excess反转特权：** 免7+条件直入池，纹理＋R:R照常
...
□ Ch5 牌型：顺势/反转/诈唬/延续？
  ...
  □ Excess牌力：单Excess(KK起步) / 双Excess(AA起步) × 共振 · 反转降一级
  □ 纹理：干燥/中性/湿润？
  □ 降级：桌风分歧?/纹理?/不确定? → 最终牌力
  □ 决策总表查表 → 🟢Go / 🔴弃牌
    ↓ Go
